...
class RapportController  extends Controller
{
    ...
    {      
        ...
        $nbrTicketsGenerated = DB::select("select p.id as id_produit,count(t.num_ticket_produit) as nbrtickets from tickets t, produits p where (t.satut <> 'annulé') and (p.id = t.id_produit  and ( DATE(t.updated_at)>=DATE('$date_debut') and DATE(t.created_at)<=DATE('$date_fin'))) group by (p.id) order by id_produit asc ");
